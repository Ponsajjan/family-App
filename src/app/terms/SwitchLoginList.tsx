...
function SwitchLoginList({ setRefetch, mainMemberNameRef }: any) {
    ...
    const handleToggleChange = async (account: string) => {
        if (mainMemberNameRef === account || switchingAccount) {
            return;
        }
        try {
                ...
                {accounts.map((account, index) => {
                    ...
                    return (
                        <div key={index} className='py-0.5 w-full'>
                            <div onClick={() => handleToggleChange(account)} className={`flex items-center justify-between transform transition-all duration-200 min-h-[40px] bg-field_color border border-l-4 ${account === mainMemberNameRef ? 'border-gray-500 text-gray-border-gray-500' : 'border-border_color text-text_color/45'} rounded-md cursor-pointer`}>
                                <div className='px-3'>{formattedName}</div>
                                <ToggleSwitch
                                    isActive={account === mainMemberNameRef}
                                    className={`${account === mainMemberNameRef ? '' : 'opacity-45'}`}
                                />
                            </div>
                        </div>
                    );
