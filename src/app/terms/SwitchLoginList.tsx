        ...
        <>
            ...
            <div className='px-4 pb-4 border-t border-dashed pt-2 mr-[0.375rem]'>
                <form onSubmit={handleSubmit}>
                    <div
                        key={errorTrigger}
                        className={`flex h-12 border border-border_color bg-field_color opacity-85 rounded-md overflow-hidden px-2 relative ${error ? 'passwordError' : ''} ${switchingAccount ? 'pulsing-shadow' : ''}`}
                    >
                        ...
                        <button
                            type='submit'
                            disabled={switchingAccount || isToggling || !form.password.trim()}
                            className={`py-1 transition-opacity duration-200 ${switchingAccount || isToggling || !form.password.trim()
