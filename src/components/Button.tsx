...
export const LinkButtonOutline = ({ buttonText, linkto = '', isLoading = false, disabled = false, className, loadingText = "Loading..." }: LinkButtonProps) => {
    const isDisabled = isLoading || disabled;
    const commonClasses = `block text-center min-w-[150px] h-10 md:h-12 px-2 md:px-4 py-2 md:py-3 text-base md:text-lg md:leading-none shadow-md rounded-md font-medium text-text_color bg-field_color border-2 border-accent_color transition-all ${isDisabled ? 'opacity-40 cursor-not-allowed pointer-events-none select-none' : 'active:shadow-none cursor-pointer'} ${className || ''}`;
