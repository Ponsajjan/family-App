  ...
  const renderEventList = (events: CalendarMonthlyEvent[], title: string) => {
    ...
    return (
      <section aria-label={title} className="w-full">
        ...
        <ul className="pl-5 md:pl-4 pb-1 list-none">
          {events.map((item, index) => {
            ...
            return (
              <li key={`${item.id}-${index}`} className="border-l border-border_color pt-2 pb-1 pl-4 pr-3">
                <div
                  role="button"
                  tabIndex={0}
                  onClick={() => setSelectedMemberId('member', item.id ?? null)}
                  onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); setSelectedMemberId('member', item.id ?? null); } }}
                  aria-label={`View ${item.name} – ${eventTypeLabel}`}
                  className="flex items-center bg-field_color text-text_color border border-l-4 border-border_color rounded-md min-h-[3.75rem] cursor-pointer"
                >
                  {item.hasDate ? (
                    <div className={`border-t border-dashed ${title === "Earlier This Month" ? 'opacity-60' : ''} border-text_color w-14 ml-2 mr-3`} aria-hidden="true">
                    ...
                    <div className="w-14 ml-2 mr-3" aria-hidden="true">
                  ...
                  <div className='w-full flex justify-between items-center'>
                    <div>
                      <div className='font-medium capitalize leading-5'>{item.name}</div>
                      <div className='text-xs font-light capitalize flex items-end gap-2'>
                        ...
                        {item.hasDate && (item.type === 'birthday' ? <Birthday2 aria-hidden="true" /> : <Deathday2 aria-hidden="true" />)}
                      </div>
                    </div>
                    {item.age !== 'n/a' && (
