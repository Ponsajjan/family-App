...
import { BurgerMenuIcon, CalendarIcon, CloseIcon, FamilyProfessionals, Moderator, RelationshipFinderIcon, RelativesIcon, Terms, TreeIcon } from "@/utils/Icons";
    ...
    return (
        <>
            ...
            <div
                ...
                className={`fixed xl:hidden ${showNav ? 'right-0 bg-gray-500/60' : 'left-full delay-500 bg-gray-300/5'} inset-0 z-[120] duration-500 ease-in-out`}
                aria-hidden="true"
            />
            <nav
                id="main-sidenav"
...
export function NavLink({ link, linkName, onClick, showDot }: { link: string, linkName: string, onClick: () => void, showDot?: boolean }) {
    ...
    return (
        ...
        >
            <span className={`group-hover:invert relative ${isActive ? "invert" : " "}`} aria-hidden="true">
                ...
                {linkName === 'Moderator' && showDot && (
                    <span className="absolute bottom-0.5 -right-0.5 flex h-2.5 w-2.5 border rounded-full" aria-label="Has pending items">
