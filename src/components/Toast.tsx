...
function ToastProvider({ children }: { children: React.ReactNode }) {
    ...
    const toastIcons: { [key in Toast["type"]]: any } = {
        success: <Success aria-hidden="true" />,
        error: <Error aria-hidden="true" />,
        warning: <Warning aria-hidden="true" />,
        info: <Info aria-hidden="true" />,
    };

    const toastTypeLabels: { [key in Toast["type"]]: string } = {
        success: 'Success',
        error: 'Error',
        warning: 'Warning',
        info: 'Info',
            ...
            >
                {toasts.map(({ id, component, type, duration }) => (
                    <div
                        key={id}
                        role="alert"
                        aria-label={`${toastTypeLabels[type]} notification`}
